################################################################################
# Title:    Meta-analyses cumulative depression exposure
...
outcomes <- c('int_', 'ext_', 'adhd_', 'asd_', 'wm_')
results_list <- list()
...
load_results <- function(path, outcome, cohort_name) {
  ...
  colnames(df) <- c("cohort", "cumul_dep_betas", "ses", "N")
...
for (outcome in outcomes) {
  ...
  if (outcome %in% c('int_', 'ext_', 'adhd_')) {
    # Load ECCN results
    MAIN_results <- read.csv(paste0("ECCN/results_", outcome, "cumul_model_ECCN.csv"))
    names(MAIN_results)[1] <- "cohort"
    colnames(MAIN_results)[colnames(MAIN_results) == "Estimate"] <- "cumul_dep_betas"
    colnames(MAIN_results)[colnames(MAIN_results) == "Std..Error"] <- "ses"
    MAIN_results <- rbind(MAIN_results,
                          load_results("ALSPAC_results/results_cumul_model_ALSPAC.csv", outcome, "alspac"))
  } else if (outcome %in% c('asd_')) {
    MAIN_results <- read.csv(paste0("ECCN/results_", outcome, "cumul_model_ECCN.csv"))
    names(MAIN_results)[1] <- "cohort"
    colnames(MAIN_results)[colnames(MAIN_results) == "Estimate"] <- "cumul_dep_betas"
    colnames(MAIN_results)[colnames(MAIN_results) == "Std..Error"] <- "ses"
  } else if (outcome %in% c('wm_')) {
    MAIN_results <- load_results("ALSPAC_results/results_cumul_model_ALSPAC.csv", outcome, "alspac")
  }
  
  # Final formatting
  MAIN_results$cumul_dep_betas <- as.numeric(MAIN_results$cumul_dep_betas)
  ...
  # Run meta-analysis
  m.model2 <- rma(yi = MAIN_results$cumul_dep_betas, sei = MAIN_results$ses)
  coefs <- coef(summary(m.model2))
  
  # Add confidence intervals
  MAIN_results <- MAIN_results %>%
    mutate(conf.low = cumul_dep_betas - 1.96 * ses,
           conf.high = cumul_dep_betas + 1.96 * ses,
           estimate = cumul_dep_betas)
...
write_combined_results_to_word <- function(results_list, filename = "cumulative_results_table.docx", cohorts) {
  outcome_names <- c("int_", "ext_", "adhd_", "asd_", "wm_")
...
write_combined_results_to_word(results_list, file.path(outdir, "cumulative_model_combined_results.docx"), cohorts)
...
format_result_beta <- function(results_list, outcome) {
  ...
  est <- round(total_row$estimate, 2)
  lower <- round(total_row$conf.low, 2)
  upper <- round(total_row$conf.high, 2)
