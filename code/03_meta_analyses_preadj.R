################################################################################
# Title:    Meta-analyses models adjusted for pre-pregnancy depression
...
for (outcome in outcomes) {
  # Load ECCN results
  MAIN_results <- read.csv(paste0("ECCN/results_", outcome, "preadj_model_ECCN.csv"))
  ...
  if (outcome %in% c('int_', 'ext_', 'adhd_')) {
    MAIN_results <- rbind(MAIN_results,
                          load_results("ALSPAC_results/results_prepregadj_model_ALSPAC.csv", outcome, "alspac"))
...
write_combined_results_to_word <- function(results_list, filename = "prepregadj_results_table.docx", cohorts) {
...
write_combined_results_to_word(results_list, file.path(outdir, "prepregadj_model_combined_results.docx"), cohorts)
