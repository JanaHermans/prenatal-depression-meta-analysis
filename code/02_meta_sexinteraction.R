################################################################################
# Title:    Meta-analyses sex interaction models 
...
load_results <- function(path, outcome, cohort_name) {
  ...
  colnames(df) <- c("cohort", "preg_dep_sex_betas", "ses", "N")
...
for (outcome in outcomes) {
  # Load ECCN results
  MAIN_results <- read.csv(paste0("ECCN/results_", outcome, "sexint_model_ECCN.csv"))
  names(MAIN_results)[1] <- "cohort"
  colnames(MAIN_results)[colnames(MAIN_results) == "preg_dep_sex_int_ses"] <- "ses"
  MAIN_results <- MAIN_results[c("cohort", "preg_dep_sex_betas", "ses", "N")]

  # Determine which cohorts to add
  if (outcome %in% c('int_', 'ext_', 'adhd_')) {
    MAIN_results <- rbind(MAIN_results,
                          load_results("ALSPAC_results/results_sexint_model_ALSPAC.csv", outcome, "alspac"),
                          load_results("PREDOresultsandscript/results_sexint_model_PREDO.csv", outcome, "predo"))
  ...
  # Final formatting
  MAIN_results$preg_dep_sex_betas <- as.numeric(MAIN_results$preg_dep_sex_betas)
  ...
  # Run meta-analysis
  m.model2 <- rma(yi = MAIN_results$preg_dep_sex_betas, sei = MAIN_results$ses)
  coefs <- coef(summary(m.model2))
  
  # Add confidence intervals
  MAIN_results <- MAIN_results %>%
    mutate(conf.low = preg_dep_sex_betas - 1.96 * ses,
           conf.high = preg_dep_sex_betas + 1.96 * ses,
           estimate = preg_dep_sex_betas)
...
write_combined_results_to_word(results_list, file.path(outdir, "sexmodel_combined_results_MH.docx"), cohorts)
...
write_combined_results_to_word(results_list, file.path(outdir, "sexmodel_combined_results_C.docx"), cohorts)
